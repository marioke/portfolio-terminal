...
export class Terminal {
    ...
    public static registerCommand(command: Command) {
        if (
            this.commands.find(
                (x) => x.name === command.name || (x.aliases && x.aliases.includes(command.name))
            )
        ) {
    ...
    private static print(text: string) {
        Terminal.writeln(`${Terminal.prefix} <span class="cmd">${text}</span>`, "no-animation", 0);
    }
    ...
    public static async writeln(text: string, style: string = "", time: number = 0) {
        // text replaceAll links
        Object.keys(links).forEach((key) => {
            const link = links[key];
            text = text.replaceAll(key, `<a href="${link}" target="_blank">${key}</a>`);
        });
    ...
    public static async writelns(name: string[], style: string = "", time: number = 30) {
        const promises = [];
